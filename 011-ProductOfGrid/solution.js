var toUse = [[08, 02, 22, 97, 38, 15, 00, 40, 00, 75, 04, 05, 07, 78, 52, 12, 50, 77, 91, 08],
...
function productOfGrid(ofNFactors) {
    ...
    for (var i = 0; i < toUse.length; i++) {
        for (var j = 0; j < toUse[i].length; j++) {
            ...
            for (var k = minCheckY; k < maxCheckY; k++) {
                if (counter == ofNFactors) {
                    ...
                    first = toUse[k + 1 - ofNFactors][currentX];
                    counter--;
                }
                var current = toUse[k][currentX];
            ...
            for (var n = minCheckX; n < maxCheckX; n++) {
                if (counter == ofNFactors) {
                    runningTotal = runningTotal / first;
                    first = toUse[currentY][n + 1 - ofNFactors];
                    counter--;
                }
                current = toUse[currentY][n];
                runningTotal *= current;
                counter++;
                if (runningTotal > largestProduct) {
                    largestProduct = runningTotal;
        ...
        for (var p = minCheckY; p < maxCheckY; p++){
            if (counter == ofNFactors) {
                runningTotal = runningTotal / first;
                first = toUse[p + 1 - ofNFactors][p + 1 - ofNFactors];
                counter--;
            }
            if (minCheckX + p < maxX){
                current = toUse[p][minCheckX + p];
                runningTotal *= current;
                counter++;
                if (runningTotal > largestProduct) {
                    largestProduct = runningTotal;
